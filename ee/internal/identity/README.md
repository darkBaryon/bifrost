...
| [session.go](session.go) | 登录、登出、会话验证和一次性票据 |
| [accounts.go](accounts.go) | 初始化、旧账号导入、创建和启停账号 |
...
账号删除由accounts.go执行：检查Users.Manage、自己和固定恢复账号保护，事务策略负责最后管理员检查及角色解绑，存储清理会话和票据；密码事件保留。session.go在成功登录事务记录last_login_at，从未登录或旧记录为null，失败不更新。
