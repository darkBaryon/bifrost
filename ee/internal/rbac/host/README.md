...
| [settings.go](settings.go) | 比较设置真正改了什么，再检查对应功能的权限；隐藏代理地址中的认证信息，并检查全局代理的凭据目标 |
...
- 携带旧凭据更换地址、代理或TLS信任设置需要 `Security.ChangeCredentialDestination`，同时仍需所属模块管理权限。主管理员默认拥有，普通角色可单独授予；不开放凭据明文读取。厂商密钥的地址覆盖与新建密钥继承旧请求头也检查；厂商代理URL中的旧认证即使被独立字段覆盖，仍按已保存凭据保护，须替换或删除才算全部使用新凭据；MCP的已保存用户令牌及令牌交换不能因换了可见密钥就视为全新凭据。
