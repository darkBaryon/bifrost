...
| [assignments.go](assignments.go) | 给账号分配角色，保护最后主管理员、删除账号前解绑角色，以及初始化和恢复时使用的账号信息与角色绑定 |
...
| [identity/policy.go](identity/policy.go) | 把账号动作对应到权限检查，将初始化、恢复绑定及删除前检查加入身份事务 |
...
| [persistence/accounts_test.go](persistence/accounts_test.go) | 账号删除、最后管理员保护、跨模块回滚和登录时间原子更新 |
...
- 不能修改自己的角色分配；移除角色、停用或删除账号时，至少保留一个启用的主管理员。启用但需改密的账号仍计入保护。
- 判权、保护检查和写入必须处于同一事务，并与身份模块共用写锁；删除账号时先检查最后管理员并解绑角色，身份存储再清理账号及会话；任一步失败整体回滚。
