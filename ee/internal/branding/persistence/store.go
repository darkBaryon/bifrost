// 本文件处理品牌配置的读取、按字段原子保存。
//
// Package persistence 负责品牌配置的存储与迁移，不处理 HTTP 请求或界面状态。
package persistence
...
func (s *BrandingStore) Read(ctx context.Context) (branding.Settings, error) {
	var row brandingRow
	err := s.db.WithContext(ctx).First(&row, 1).Error
	return row.settings(), err
}

// Update 只更新 patch 指定的图片位置，并在同一事务中读取保存结果。
// 任一步失败都会回滚，未指定的位置保持原值。
func (s *BrandingStore) Update(ctx context.Context, patch branding.Patch) (branding.Settings, error) {
	var result brandingRow
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		row := brandingRow{ID: 1, UpdatedAt: time.Now().UTC()}
...
var _ branding.Repository = (*BrandingStore)(nil)
