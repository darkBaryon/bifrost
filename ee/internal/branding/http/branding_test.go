...
func TestBrandingInvalidPayloadAtomic(t *testing.T) {
	...
	if brandingRequest(r, "POST", "/api/branding/update", `{"logo":"`+strings.Repeat("A", base64.StdEncoding.EncodedLen(branding.MaxImageBytes)+4)+`"}`, nil).Response.StatusCode() != 413 {
		t.Fatal("image limit")
	}
	oversized := imageBytes(t, "png", 4097, 1)
	if _, err := decodeBrandingAsset(json.RawMessage(`"`+base64.StdEncoding.EncodeToString(oversized)+`"`), nil); err == nil {
		t.Fatal("dimension limit")
	}
	truncated := imageBytes(t, "png", 2, 2)
	truncated = truncated[:len(truncated)-12]
	if _, err := decodeBrandingAsset(json.RawMessage(`"`+base64.StdEncoding.EncodeToString(truncated)+`"`), nil); err == nil {
...
// 本次迁移前固定空编码、两图错误顺序与资源路径优先级，防止拆层改变已有行为。
func TestBrandingValidationPrecedence(t *testing.T) {
	r, _, db := brandingTestRouter(t)
	good := base64.StdEncoding.EncodeToString(imageBytes(t, "png", 2, 2))
	before := decodeResponse(t, brandingRequest(r, "POST", "/api/branding/update", `{"logo":"`+good+`"}`, nil))
	for _, body := range []string{
		`{"logo":"data:;base64,"}`,
		`{"logo":"\r\n"}`,
		`{"logo":"bm90LWFuLWltYWdl","icon":"` + strings.Repeat("A", base64.StdEncoding.EncodedLen(branding.MaxImageBytes)+4) + `"}`,
	} {
		if ctx := brandingRequest(r, "POST", "/api/branding/update", body, nil); ctx.Response.StatusCode() != 400 {
			t.Fatalf("invalid input status=%d body=%s", ctx.Response.StatusCode(), ctx.Response.Body())
		}
		if after := decodeResponse(t, brandingRequest(r, "POST", "/api/branding/get", "", nil)); after != before {
			t.Fatal("invalid input changed existing state")
		}
	}
	sql, _ := db.DB()
	sql.Close()
	for path, want := range map[string]int{
		"/api/branding/assets/other/" + strings.Repeat("a", 64): 404,
		"/api/branding/assets/logo/short":                       404,
		"/api/branding/assets/logo/" + strings.Repeat("a", 64):  500,
	} {
		if got := brandingRequest(r, "GET", path, "", nil).Response.StatusCode(); got != want {
