...
func (s *Store) Update(ctx context.Context, configJSON string, expectedVersion int) (Row, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&Row{}).Where("id = ? AND version = ?", singletonID, expectedVersion).
			Updates(map[string]any{"config": result.Config, "version": result.Version, "updated_at": result.UpdatedAt}).Error
	})
	return result, err
}
