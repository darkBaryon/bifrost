...
- **判官（D1/D4/D5）**：每个项目一个实例，每条业务规则一个实例；判官请求是跳过插件管线的内部子请求，不会再经过本插件，也不进上游日志与计费（本包只记 token 数）。判官返回的理由不记录。判官失败时，日志与错误链不含送检内容或 provider 返回的自由文本。先看网关自己的标记：网关标记为自身错误的（取消、超时，带不带状态码都算）或根本没有状态码的（key 池没有支持判官模型的 key）保留类型、代码与前 200 字节消息；其余带状态码的错误只留状态码和按状态码派生的固定类别，类型命中网关合成清单时附加该常量名。判别规则见 [host/judge.go](host/judge.go) 的 `judgeError`。判官 provider 自带的重试会与检测器重试相乘，装配时记 Warn。
...
- **文本边界：** 输入只检查最后一条消息（客户端会把被拦的消息留在历史里，检查全部历史会让之后每条请求都被同一句话拦下；历史里的每条消息在它到达时已检查过）；输出逐个检查候选回答，所有候选共享总字节预算。文本必须是有效 UTF-8，空文本仍送检。
