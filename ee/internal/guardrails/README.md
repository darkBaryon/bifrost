...
- **支持范围：** 当前只检查规范化的纯文本 Chat。未知参数、工具、多模态、拒答正文等未支持内容会被拒绝；推理模型的推理正文会和回答一起检查（加密推理跳过）；完整判断见 [plugin/text.go](plugin/text.go)。超大请求或响应的原样透传模式也不支持，配置大响应透传阈值同样会被拒绝。
- **文本边界：** 输入按规则的 `Scope` 取文本：判官类规则只看最后一条消息（客户端会把被拦的消息留在历史里，查全部历史会让之后每条请求都被同一句话拦下；历史里的每条消息在它到达时已检查过），密钥规则看全部消息（历史里的密钥每一轮都在往外发，输出检查兜不住）；输出逐个检查候选回答，所有候选共享总字节预算。文本必须是有效 UTF-8，空文本仍送检。
