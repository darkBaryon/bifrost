...
bifrost fork（darkBaryon/bifrost）的本地功能开发工作台：在文件系统里管理需求、方案、评审、验收的流转，并渲染成一个干净的本地站点。**只管开发流程，不含业务代码。**

- **流程规范**（共享机制 + 类型差异，2026-09 本地重排）：[规范.md](规范.md)（主页/理念/页面地图）+ [规范/流程.md](规范/流程.md)（执行序唯一权威）
