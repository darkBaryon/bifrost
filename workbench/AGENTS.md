...
- **收敛评审**（规范/change/convergence-review.md）：预飞通过后与代码评审并行、先于 Gate 2，**每个 Change 必经**，重量随风险分级（L1 一两句并入交付摘要；L2 起单独记录）。评审者须与实施上下文隔离。SCHEMA 未收录 `收敛评审` 类型前，暂借 `代码评审` 类型落档并在正文首段注明。（此条 2026-09-02 补：UI完整中文化期1 因锚点缺失漏做收敛评审，补做后修订）
