...
本工作台是本项目功能开发的过程文档库（需求 + 过程文档两类，存在文件系统、渲染成本地站点）。**规范全文**在 `规范.md`（主页）+ `规范/` 四套规范（Playbook v1.1.0 只读副本）；本文件只规定「在本仓怎么操作」。
...
- `evidence/`：证据类附件（如自动生成的分流清单、大体量数据核实记录）不套六种正式类型的状态机时存放于此，不在 `build_views.py` 的 frontmatter 校验范围内。命名 `<case>-期<N>-<用途>.md`；方案/验收记录中按相对路径引用。
...
- **收敛评审**（规范/change/convergence-review.md）：预飞通过后与代码评审并行、先于 Gate 2，**每个 Change 必经**，重量随风险分级（L1 一两句并入交付摘要；L2 起单独记录）。评审者须与实施上下文隔离；出口三态（无碍/仅立债挂账/有Blocking）落 frontmatter `verdict`，立债/挂账同步登记 `findings.md` 台账。（此条 2026-09-02 补：UI完整中文化期1 因锚点缺失漏做收敛评审，补做后修订）
