...
本目录面向具体开发人员，管理实施方案、代码改动设计、任务步骤、测试、评审和交付记录。产品经理和技术负责人级别的需求、路线图、整体架构与长期技术决策放在 `../product/`；开发前读取并引用对应输入，不在此另建一套整体架构。具体分工见 [README](README.md)。

本工作台是本项目功能开发的过程文档库（需求 + 过程文档两类，存在文件系统、渲染成本地站点）。**规范全文**在 `规范.md`（主页/页面地图）+ `规范/`（共享流程 + 类型差异，本地演化）；**执行序唯一权威在 [规范/流程.md](规范/流程.md)**；本文件是唯一操作页——在本仓怎么读、怎么写全在这里。
...
## 流程锚点（执行序以 [规范/流程.md](规范/流程.md) 为唯一权威，此处只列本仓落地信号）
